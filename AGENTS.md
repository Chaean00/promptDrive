...
- Java `record`와 Lombok `@Value`를 사용하지 않는다. 요청·응답 DTO와 이벤트 payload는 필요한 Lombok 어노테이션만 선택하고, `private final` 필드·명시적 생성자·정적 팩토리 메서드로 불변 클래스를 작성한다. JPA Entity는 용도에 맞는 Lombok 어노테이션으로 작성한다.
